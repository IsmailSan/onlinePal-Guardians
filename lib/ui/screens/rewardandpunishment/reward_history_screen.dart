...
class RewardHistoryScreen extends StatefulWidget {
  final RewardHistoryItem? reward;
  final String? status;

  const RewardHistoryScreen({Key? key, this.status, this.reward})
      : super(key: key);
...
class _RewardHistoryScreenState extends State<RewardHistoryScreen> {
  ...
  String missionCondition = 'Setiap Kali Misi Gagal';
  late final RewardHistoryItem? _historyReward;
  ...
  Future<void> _initializeData() async {
    ...
    _historyReward = widget.reward;
    _initialStartDate = DateTime.parse(
        _historyReward?.periodStartDate ?? DateTime.now().toIso8601String());
    _initialEndDate = DateTime.parse(
        _historyReward?.periodEndDate ?? DateTime.now().toIso8601String());
    _pointController.text = _historyReward?.pointsNeeded ?? "";
    _noteController.text = _historyReward?.description ?? "";
    _missionSuccessCountController.text = _historyReward?.qtyCondition.toString() ?? "";
    _missionController.text = _historyReward?.mission?.name  ?? "";
    _rewardAssignedTimeController.text = _historyReward?.createdAt  ?? "";
    _exchangedPointsController.text = _historyReward?.pointsNeeded  ?? "";
                          ...
                          children: [
                            ...
                            BlocListener<RewardPunishmentBloc, RewardPunishmentState>(
                              listener: (context, state) {
                                if (state is ConfirmRewardSuccess) {
                                  showDialog(
                                    context: context,
                                    barrierDismissible: false,
                                    builder: (_) => SuccessDialog(message: 'Hadiah berhasil ditandai sebagai selesai', onOk: () {
                                      ...
                                      showDialog(
                                        ...
                                        builder: (_) =>
                                            ConfirmActionDialog<RewardPunishmentBloc>(
                                              title: 'Apakah Anda yakin ingin menandai hadiah ini sebagai "selesai"?',
                                              subtitle:
                                              ...
                                              onConfirmEvent:
                                              ConfirmReward(rewardId: widget.reward?.id ?? 0),
                                            ),
                                      );
                                    },
                                    ...
                                    showDialog(
                                      ...
                                      builder: (_) =>
                                          ConfirmActionDialog<RewardPunishmentBloc>(
                                            ...
                                            secondaryButtonColor: whiteColor,
                                            onConfirmEvent:
                                            DeleteReward(rewardId: widget.reward?.id ?? 0),
                                          ),
                                    );
                                  },
                                  style: OutlinedButton.styleFrom(
